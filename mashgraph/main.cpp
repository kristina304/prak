...
int main()
{
    ...
    //чтобы не было границ видимости карты глубины
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_BORDER);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_BORDER);
    float borderColor[] = { 1.0, 1.0, 1.0, 1.0 };
    glTexParameterfv(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR, borderColor);
    // прикрепл€ем текстуру глубины в качестве буфера глубины дл€ FBO
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //матрица ортографической проекции, дл€ того чтобы направленный свет не исказил перспективу
        lightProjection = glm::ortho(-10.0f, 10.0f, -10.0f, 10.0f, near_plane, far_plane);
        //матрица вида дл€ преобразовани€
        ...
        landscape.setMat4("view", view);
        landscape.setMat4("projection", projection);         
        glBindVertexArray(skyboxVAO);
        glActiveTexture(GL_TEXTURE0);
...
unsigned int loadTexture(char const * path)
{
    ...
    if (data)
    {
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, format == GL_RGBA ? GL_CLAMP_TO_EDGE : GL_REPEAT); // дл€ данного урока: используйте GL_CLAMP_TO_EDGE дл€ предотвращени€ возникновени€ полупрозрачных границ. Ѕлагодар€ интерпол€ции берутс€ тексели из следующего повторени€ 
